...
from mmdet.core.bbox.iou_calculators import bbox_overlaps as torch_bbox_overlaps
...
def get_in_gt_and_in_center_info(priors, gt_bboxes, center_radius):
    ...
    if priors.is_cuda:
        ...
        simota_cuda_ops.check_prior_in_gt(
            priors, gt_bboxes, is_in_gts, is_in_cts, center_radius
        )
...
def get_cls_cost(valid_pred_scores, gt_labels):
    '''
    (large input_size, large num_gt) -> large num_valid
    (large num_valid, large num_gt) ->
        high F.binary_cross_entropy(..., reduction='none') memory cost
    Use CUDA extension to reduce the cost (to 1/num_classes)
    '''
    ...
    if valid_pred_scores.is_cuda:
        ...
        gt_onehot_labels = F.one_hot(
            gt_labels.to(torch.int64), valid_pred_scores.shape[-1]
        ).float()
        simota_cuda_ops.binary_cross_entropy_cost(
            valid_pred_scores.sqrt_(), gt_onehot_labels, cls_cost
        )
    else:
        gt_onehot_label = (
...
class SimOTAAssigner(BaseAssigner):
    ...
    def assign(
        ...
        try:
            ...
            print(priors.shape, gt_bboxes.shape)
    ...
    def get_in_gt_and_in_center_info(self, priors, gt_bboxes):
        return get_in_gt_and_in_center_info(
            priors, gt_bboxes, self.center_radius
        )
